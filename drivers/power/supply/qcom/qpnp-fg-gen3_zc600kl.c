...
#include <linux/alarmtimer.h>//Bug 305574, baidabin.wt,MODIFY,2017-10-08,add asus debug info
#include <linux/switch.h> //ASUS_BSP register battery version
#include "fg-core.h"
#include "fg-reg.h"
...
struct fg_chip * g_fgChip = NULL; //ASUS_BSP 
...
//+ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for hot temp recharger voltage
bool asus_hot_temp_recharger = false;
//-ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for hot temp recharger voltage
/* All getters HERE */
...
static int fg_charge_full_update(struct fg_chip *chip)
{
	...
	if (chip->charge_done && !chip->charge_full) {
		if (msoc >= 99 && chip->health == POWER_SUPPLY_HEALTH_GOOD) {
			 ...
			 */
//+ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for hot temp recharger voltage
			if (false == asus_hot_temp_recharger)
			{
...
//+Bug 305574, baidabin.wt,MODIFY,2017-10-08,add asus debug info
static void asus_debug_charge_info(struct fg_chip *chip)
{
	struct timespec ts;
	struct rtc_time tm;
	int rc = 0;
	...
	rc = fg_get_prop_capacity(chip, &bat_soc);
	if (rc < 0) {
	...
	rc = fg_get_battery_current(chip, &bat_curr);
	if (rc < 0) {
    ...
    //+Bug 312285, baidabin.wt,MODIFY,2017-10-24, add asus debug info
	pr_err("ASUS_CHG_INFO: BAT_FULL(%d), HEALTH(%d), VOL(%d), CURRENT(%d), SOC(%d), CHG_TYPE(%d), TEMP(%d), TIME(%02d-%02d %02d:%02d:%02d)\n",
		chip->charge_full, chip->health, bat_vol, bat_curr, bat_soc,
		chip->charge_type, batt_temp, tm.tm_mon+1, tm.tm_mday+(tm.tm_hour+8)/24, (tm.tm_hour+8)%24, tm.tm_min, tm.tm_sec);
    //-Bug 312285, baidabin.wt,MODIFY,2017-10-24, add asus debug info
}
//-Bug 305574, baidabin.wt,MODIFY,2017-10-08,add asus debug info
...
static void status_change_work(struct work_struct *work)
{
	...
	fg_ttf_update(chip);
	asus_debug_charge_info(chip);//Bug 305574, baidabin.wt,MODIFY,2017-10-08,add asus debug info
...
//+ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for hot temp recharger voltage	
#define ASUS_HOT_TEMP_RECHARGER_VOLT_THR_MV 3907
//-ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for hot temp recharger voltage	
...
//+ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for hot temp recharger voltage	
	pr_err("fg_delta_batt_temp_irq_handler batt_temp=%d, hot_recharger=%d\n", batt_temp,asus_hot_temp_recharger);
	if ((batt_temp >= 500) && (false == asus_hot_temp_recharger))
	{
		pr_err("hot temp fg_set_recharge_voltage.\r\n");
		rc = fg_set_recharge_voltage(chip, ASUS_HOT_TEMP_RECHARGER_VOLT_THR_MV);
		if (rc < 0)
		{
			pr_err("Error in hot temp fg_set_recharge_voltage rc:%d\n", rc);
		} 
        else
		{
		    asus_hot_temp_recharger = true;
		}

	}
	else if ((batt_temp < 500) && (true == asus_hot_temp_recharger)) 
	{
		pr_err("normal temp fg_set_recharge_voltage.\r\n");
		rc = fg_set_recharge_voltage(chip,chip->dt.recharge_volt_thr_mv);
		if (rc < 0)
		{
			pr_err("Error in normal temp fg_set_recharge_voltage rc:%d\n", rc);
		} 
		{
			asus_hot_temp_recharger = false;
		}
	}
//-ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for hot temp recharger voltage
...
static int print_battery_status(void) {
	...
	pr_info("%s: %s", __func__, battInfo);	
	pr_info("%s: %s", __func__, additionBattInfo);
...
void static update_gauge_status_worker(struct work_struct *dat)
{
	...
	if (!g_fgChip->profile_loaded) {
		pr_info("%s: profile not loaded yet, delay 5s\n", __func__);
	...
	*/
		...
		if (ret == 0) {
			...
			pr_info("%s: charger not ready yet, delay 5s\n", __func__);
			qpnp_smbcharger_polling_data_worker(5);
		}
	//}
}
...
//ASUS_BSP +++ Register battery version 
struct switch_dev batt_dev;
static ssize_t batt_switch_name_ZC600KL(struct switch_dev *sdev, char *buf)
{
	const char* bat_driverVersion = fg_get_battery_type(g_fgChip);
...
void register_battery_version(void)
{
	...
	batt_dev.print_name = batt_switch_name_ZC600KL;
	if (switch_dev_register(&batt_dev) < 0) {
		pr_err("%s: fail to register battery switch\n", __func__);
	}
}
//ASUS_BSP --- Register battery version 
//ASUS_BSP ---
...
static int fg_gen3_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&update_gauge_status_work, update_gauge_status_worker); //ASUS_BSP
	...
	register_battery_version(); //ASUS_BSP register battery version
