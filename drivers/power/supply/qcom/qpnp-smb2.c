...
#define TITAN_INOV_TRIGGER		45
#define TITAN_INOV_RELEASE		43
//ASUS for shipping ---
...
//WeiYu: setting inov ++
	smblib_write(chg, SKIN_HOT_REG, (TITAN_INOV_RELEASE+30)*2);
	smblib_write(chg, SKIN_TOO_HOT_REG, (TITAN_INOV_TRIGGER+30)*2);
//WeiYu: setting inov --
...
void asus_regist_adc_gpios(struct platform_device *pdev){
	...
	if (rc)
		CHG_DBG_E("%s: failed to request ADC_SW_EN-gpios59 %d\n", __func__,(int)gpio_ctrl->ADC_SW_EN);
	else
	...
	if (rc)
		CHG_DBG_E("%s: failed to request ADCPWREN_PMI_GP1-gpios34 %d\n", __func__,(int)gpio_ctrl->ADCPWREN_PMI_GP1);
	else
