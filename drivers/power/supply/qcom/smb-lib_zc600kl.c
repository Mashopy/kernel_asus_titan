...
#include "linux/syscalls.h" //+Bug321118 baidabin.wt,MODIFY,20171129,modify country code detect issue
#include "smb-lib.h"
#include "smb-reg.h"
#include "battery.h"
#include "step-chg-jeita.h"
#include "storm-watch.h"
bool lcm_is_off = false;//+Bug 305574, baidabin.wt,ADD,2017-10-08,skip thermal chg current set when display is off
bool asus_dcp_detect_flag = 0;//Bug314787,zhaosidong.wt,MODIFY,20171031,modify discontinuous charging problems
//+Bug321118 baidabin.wt,MODIFY,20171129,modify country code detect issue
int asus_country_code_detect_times = 0; 

#define CODE_UNDEFINED		-1
#define CODE_BR				1
#define CODE_OTHER			0
int asus_country_code_is_br = CODE_UNDEFINED;
bool asus_support_fast_charge = false; ////Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
//+Bug 329838,baidabin.wt,MODIFY,2017-12-15, Add the global variable
//bool asus_cold_limit_current = false; //+Bug 331298 ,baidabin.wt,DELETE,2017-12-21, remove global variable 
bool asus_high_voltage_limit_current = false;
bool asus_high_temp_limit_voltage = false;

//+Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
bool asus_float_charger_rerun = false;
//-Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
//-Bug 329838,baidabin.wt,MODIFY,2017-12-15, Add the global variable
//-Bug321118 baidabin.wt,MODIFY,20171129,modify country code detect issue

#define ASUS_CHARGER_SEL	55
#define ASUS_ADAPTER_CONTROL	62

int  g_fv_setting = 0x74;

int asus_fake_temp = FAKE_TEMP_DISABLE;
enum JEITA_state {
	JEITA_STATE_INITIAL,
...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	smblib_err(chg, "APSD=%s PD=%d\n",	//Bug 305574, baidabin.wt MODIFY,2017-10-08,open some debug message
...
//+Bug334281,zhaosidong.wt,MODIFY,20180109,delay detection charger id
    if(wake_lock_active(&chg->asus_chrg_detect_wake_lock)) {
        wake_unlock(&chg->asus_chrg_detect_wake_lock);
    }
	cancel_delayed_work_sync(&chg->country_code_detect_work);
//-Bug334281,zhaosidong.wt,MODIFY,20180109,delay detection charger id
...
int smblib_rerun_apsd_if_required(struct smb_charger *chg)
{
	union power_supply_propval val;
	int rc;
	const struct apsd_result *apsd_result = smblib_get_apsd_result(chg);   //EXTB asus Bug1152292,zhaosidong.wt,ADD,20180131,do not rerun apsd while apsd result is sdp
...
//EXTB asus Bug1152292,zhaosidong.wt,ADD,20180131,do not rerun apsd while apsd result is sdp
	return 0;
}
			...
			int hvdcp_enable, const char *client)
{
	 ...
	 * negotiation happens.
	 */
//+Bug 305574, baidabin.wt MODIFY,2017-10-08,for force disable hvdcp high voltage charging.
//	if (!hvdcp_enable)
	val = 0;	//the val before is HVDCP_EN_BIT;
//-Bug 305574, baidabin.wt MODIFY,2017-10-08,for force disable hvdcp high voltage charging.
...
#define USB_TEMP_DET_NOTIFY_MS		10000   //Bug335236,zhaosidong.wt,ADD,20180117,usb thermal alert
...
//+Bug335236,zhaosidong.wt,ADD,20180117,usb thermal alert
	cancel_delayed_work_sync(&chg->usb_temp_det_work);
	schedule_delayed_work(&chg->usb_temp_det_work, msecs_to_jiffies(USB_TEMP_DET_NOTIFY_MS));
//-Bug335236,zhaosidong.wt,ADD,20180117,usb thermal alert
...
//+Bug335236,zhaosidong.wt,ADD,20180117,usb thermal alert
	cancel_delayed_work_sync(&chg->usb_temp_det_work);
	switch_set_state(&chg->sdev, 0);
	chg->pre_otgchg_temp_enable=1;
//-Bug335236,zhaosidong.wt,ADD,20180117,usb thermal alert
				...
				union power_supply_propval *val)
{
	...
	if (val->intval != POWER_SUPPLY_STATUS_CHARGING)
		return 0;
//+ExtB 1114107, liyun2.wt, MODIFY, 20171206, for update battery status full when soc 100
	if (val->intval == POWER_SUPPLY_STATUS_CHARGING) {
...
//+Bug314787,zhaosidong.wt,MODIFY,20171031,modify discontinuous charging problems
	if (!stat){
		if(asus_dcp_detect_flag == 1){
            val->intval = POWER_SUPPLY_STATUS_CHARGING;
		}else{
		    val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
		}
				...
				union power_supply_propval *val)
{
	...
	//ASUS fake temp
	if(psp == POWER_SUPPLY_PROP_TEMP && asus_fake_temp != FAKE_TEMP_DISABLE){
		val->intval = asus_fake_temp;
		rc=0;
		CHG_DBG("report fake temp: %d\n", asus_fake_temp);
		return rc;
	}	
				...
				const union power_supply_propval *val)
{
	...
	if (val->intval > chg->thermal_levels)
		return -EINVAL;
//+Bug336811,zhaosidong.wt,ADD,20180126,usb thermal alert do not recover charging
	if(chg->thermal_offline == 1)
		return 0;
//-Bug336811,zhaosidong.wt,ADD,20180126,usb thermal alert do not recover charging
//Bug+321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
	if (false == asus_support_fast_charge)
	{
        smblib_dbg(chg, PR_MISC, "For 1A charger , Not support thermal.\n");
		return 0;
	}
//Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
...
//+Bug 305574, baidabin.wt MODIFY,2017-10-08,skip thermal chg current set when display is off
	if (lcm_is_off) {
		smblib_dbg(chg, PR_MISC, "Thermal set fcc current to 0 level\n");
		vote(chg->fcc_votable, THERMAL_DAEMON_VOTER, true,
			chg->thermal_mitigation[0]);
	}
	else {
		smblib_dbg(chg, PR_MISC, "Thermal set fcc current to %d level\n", chg->system_temp_level);
		vote(chg->fcc_votable, THERMAL_DAEMON_VOTER, true,
...
int smblib_set_prop_fcc_delta(struct smb_charger *chg, int jeita_cc_delta_ua)
{
	...
	if ((jeita_cc_delta_ua > chg->param.jeita_cc_comp.max_u)||(jeita_cc_delta_ua < chg->param.jeita_cc_comp.min_u))
	{
	    smblib_err(chg, "The jeita_cc_delta_ua is invalid.\n");
	    return 0;
	}

    rc = smblib_set_charge_param(chg, &chg->param.jeita_cc_comp, jeita_cc_delta_ua);
	if (rc < 0) {
		dev_err(chg->dev, "Error in setting jeita_cc_delta_ua rc=%d\n", rc);
		return rc;
	}
...
void asus_plug_init(struct smb_charger *chg){
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	power_supply_changed(chg->usb_psy);
	CHG_DBG_EVT("%s : IRQ: usbin-plugin %s\n",	//Bug 305574, baidabin.wt MODIFY,2017-10-08,open some debug message
...
#define USBCHG_TBUS_ALARM_GPIO		77
//+Bug336640,zhaosidong.wt,MODIFY,20180124,add for usb thermal alert
static int smb2_thermal_set_offline(struct smb_charger *chg, int value)
{
	chg->thermal_offline=value;

	return 0;
}
//-Bug336640,zhaosidong.wt,MODIFY,20180124,add for usb thermal alert
static int usb_thermal_enable_charging(struct smb_charger *chg, bool enable)
{
	union power_supply_propval val;
	union power_supply_propval usb_thermal_enable;  //Bug336966,zhaosidong.wt,ADD,20180126,usb thermal alert
	int rc = 0;
	bool usbchg_enable = enable;
	...
	if (!val.intval) //usb removed
		return rc;

	if (chg->pre_usbchg_temp_enable == usbchg_enable)
		return rc;
//+Bug336966,zhaosidong.wt,ADD,20180126,usb thermal alert
	CHG_DBG("pre_enable = %d, new_enable  = %d\n", chg->pre_usbchg_temp_enable, usbchg_enable);
	chg->pre_usbchg_temp_enable = usbchg_enable;
    usb_thermal_enable.intval = !usbchg_enable;
    rc = smblib_set_prop_input_suspend(chg, &usb_thermal_enable);

	smb2_thermal_set_offline(chg,1);
	power_supply_changed(chg->usb_psy);
//-Bug336966,zhaosidong.wt,ADD,20180126,usb thermal alert
	return rc;
}

static int usb_thermal_otg_regulator_enable(struct smb_charger *chg, bool enable)
{
	u8 stat;
	int rc = 0;
	bool otgchg_enable = enable;

	rc = smblib_read(chg, OTG_STATUS_REG, &stat);
	if (rc < 0) {
		smblib_err(chg, "Cread otg status failed ! rc=%d\n", rc);
		return rc;
	}

	if (!(stat & BOOST_SOFTSTART_DONE_BIT)) //otg removed
		return rc;

	if (chg->pre_otgchg_temp_enable == otgchg_enable)
		return rc;

	chg->pre_otgchg_temp_enable = otgchg_enable;
	rc = smblib_write(chg, CMD_OTG_REG, otgchg_enable);
	if (rc < 0) {
		smblib_err(chg, "Couldn't enable OTG rc=%d\n", rc);
		return rc;
	}
...
static int usb_thermal_check_usb_port_temp(struct smb_charger *chg)
{
	union power_supply_propval val;
	u8 stat;
	int rc = 0;
	int usbchg_gpio = 0;
	static bool usbchg_temp_high = false;
	static bool otgchg_temp_high = false;

	if (gpio_is_valid(USBCHG_TBUS_ALARM_GPIO))
		usbchg_gpio = gpio_get_value(USBCHG_TBUS_ALARM_GPIO);
	...
	if (val.intval) { //usb present
		if (usbchg_gpio == 1) { //high
			usbchg_temp_high = true;
		} else if (usbchg_gpio == 0) { //normal
			usbchg_temp_high = false;
		}
	} else { //usb remove
		usbchg_temp_high = false;
	}

	if(true == usbchg_temp_high)   //Bug335236,zhaosidong.wt,ADD,20180117,usb thermal alert
	    usb_thermal_enable_charging(chg, !usbchg_temp_high);

	rc = smblib_read(chg, OTG_STATUS_REG, &stat);
	if (rc < 0) {
		smblib_err(chg, "read otg status failed ! rc=%d\n", rc);
	}

	if (stat & BOOST_SOFTSTART_DONE_BIT) {
		if (usbchg_gpio == 1) { //high
			otgchg_temp_high = true;
		} else if (usbchg_gpio == 0) { //normal
			otgchg_temp_high = false;
		}
	} else { //otg remove
		otgchg_temp_high = false;
	}

	if (true == otgchg_temp_high)   //Bug335236,zhaosidong.wt,ADD,20180117,usb thermal alert
	    usb_thermal_otg_regulator_enable(chg, !otgchg_temp_high);

	if (otgchg_temp_high|| usbchg_temp_high ) 
		chg->usbchg_temp_alert = true;
	else
		chg->usbchg_temp_alert = false;
	

	if (chg->usbchg_temp_alert != chg->pre_usbchg_temp_alert) {
		chg->pre_usbchg_temp_alert = chg->usbchg_temp_alert;
		CHG_DBG_EVT("set switch usb alert value=%d,gpio %d, usb %d, otg %d\n",
			chg->usbchg_temp_alert,usbchg_gpio, usbchg_temp_high ,otgchg_temp_high);

		switch_set_state(&chg->sdev, chg->usbchg_temp_alert);
	}

	return rc;
}

static void usb_temp_detect_callback(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb_charger *chg = container_of(dwork,
					struct smb_charger, usb_temp_det_work);

	usb_thermal_check_usb_port_temp(chg);
	schedule_delayed_work(&chg->usb_temp_det_work, msecs_to_jiffies(USB_TEMP_DET_NOTIFY_MS));
}
//-Bug305851, zk-zhouying.wt, ADD, 2017-10-10, add usb temp alarm

void reset_asus_params(void){

	asus_high_voltage_limit_current = false;
	asus_high_temp_limit_voltage = false;	
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	struct smb_irq_data *data;
	struct storm_watch *wdata;
	union power_supply_propval usb_thermal_enable = {0,}; //Bug336966,zhaosidong.wt,ADD,20180126,usb thermal alert
...
//+Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
		asus_float_charger_rerun = false;
//-Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
...
//+Bug305851, zk-zhouying.wt, ADD, 2017-10-10, add usb temp alarm
	if (vbus_rising) {
		...
		switch_set_state(&chg->sdev, 0);
//+Bug336966,zhaosidong.wt,ADD,20180126,usb thermal alert
		smb2_thermal_set_offline(chg,0);
		smblib_set_prop_input_suspend(chg, &usb_thermal_enable);
		chg->pre_usbchg_temp_enable=1;
		gpio_direction_output(ASUS_ADAPTER_CONTROL, 0);
		gpio_direction_output(ASUS_CHARGER_SEL, 0);
		asus_support_fast_charge = false;
		reset_asus_params();
//-Bug336966,zhaosidong.wt,ADD,20180126,usb thermal alert
	}
//-Bug305851, zk-zhouying.wt, ADD, 2017-10-10, add usb temp alarm
...
//+Bug317596,zhaosidong.wt,MODIFY,20171109,determine wakelock activation status
#ifdef WT_COMPILE_FACTORY_VERSION
    if(vbus_rising && !wake_lock_active(&chg->ato_chrg_wake_lock)) {
		wake_lock(&chg->ato_chrg_wake_lock);
	} else if(!vbus_rising && wake_lock_active(&chg->ato_chrg_wake_lock)) {
	    wake_unlock(&chg->ato_chrg_wake_lock);
	}		
#endif
//-Bug317596,zhaosidong.wt,MODIFY,20171109,determine wakelock activation status

	power_supply_changed(chg->usb_psy);
	CHG_DBG_EVT("IRQ: usbin-plugin %s\n",	//Bug 305574, baidabin.wt MODIFY,2017-10-08,open some debug message
...
static void smblib_force_legacy_icl(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
								USB_PSY_VOTER))
			vote(chg->usb_icl_votable, USB_PSY_VOTER, true, 500000);//Bug 305574, baidabin.wt MODIFY,2017-10-08,modify input current limit
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, false, 0);
		break;
	case POWER_SUPPLY_TYPE_USB_CDP:
//+Bug333324,zhaosidong.wt,MODIFY,20180102,modify charging current for CDP type
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1500000);//Bug 308696,baidabin.wt,MODIFY,2017-10-13,Modify for Jeita requirement
        vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1500000);
//-Bug333324,zhaosidong.wt,MODIFY,20180102,modify charging current for CDP type
		break;
	case POWER_SUPPLY_TYPE_USB_DCP:
		...
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1000000);//Bug 308696,baidabin.wt,MODIFY,2017-10-13,Modify for Jeita requirement
...
#define ASUS_COUNTRY_CODE_DETECT_MS 15*1000    //Bug334281,zhaosidong.wt,MODIFY,20180109,delay detection charger id
#define ASUS_BATTERY_TEMP_VOLTAGE_MS 60*1000
//+Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
#define ASUS_FLOAT_CHARGER_RERUN_MS 3*1000
//-Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
#define ASUS_COUNTRY_CODE_STRING_LEN 64
#define ASUS_COUNTRY_CODE_PATH "/factory/COUNTRY"


static int asus_country_code_read(char* buf)
{
    int fhandle;
    char fbuf[ASUS_COUNTRY_CODE_STRING_LEN];
    mm_segment_t fs;
    int len = 0;

    pr_err("asus_country_code_read enter.\n");
    
	asus_country_code_detect_times++;

	fs = get_fs();
    set_fs(KERNEL_DS);

    fhandle = sys_open(ASUS_COUNTRY_CODE_PATH, O_RDONLY, 0);
    if(fhandle < 0){
	    pr_err("%s: open asus country code error, fhandle is %d\n", __func__, fhandle);
		return fhandle;
	}

    sys_lseek(fhandle, 0, 0);
	sys_read(fhandle, fbuf, ASUS_COUNTRY_CODE_STRING_LEN);

	set_fs(fs);
    sys_close(fhandle);

	len = strlen(fbuf);
	pr_err("read buf len:%d, fbuf:%s\n", len,fbuf);
...
//+Bug334281,zhaosidong.wt,MODIFY,20180109,delay detection charger id
static void asus_charger_id_detected(struct smb_charger *chg)
{
	int64_t vdm_1, vdm_2 = 0;
	int rc = 0;
	int ret = 0;
	char buff[ASUS_COUNTRY_CODE_STRING_LEN];
	struct qpnp_vadc_result ausu_vadc_result;

    /*baidabin add for detect the country code*/
    if (CODE_UNDEFINED == asus_country_code_is_br) {
        ret = asus_country_code_read(buff);
	    pr_err("read buf buff:%s\n",buff);
		if (ret < 0) {
			pr_err("%s, read asus country code fail, buf:%s\n", __func__, buff);
		}
		
		if (!strncmp(buff, "BR", 2)) {
			asus_country_code_is_br = CODE_BR;	
			pr_err("%s, The country code is BR, not support id detected.\n", __func__);
		}
		else if (ret >=0){
			asus_country_code_is_br = CODE_OTHER;

		}
	}
	/*For BR, Set Current is 2A*/
	if (CODE_BR == asus_country_code_is_br) {
		vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 2000000);
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 2000000);
		asus_support_fast_charge = true; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
		smblib_set_prop_fcc_delta(chg,500000); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
		CHG_DBG_EVT( "The country is BR, Set current as 2A.(support_fast_charge)\n");
		if(wake_lock_active(&chg->asus_chrg_detect_wake_lock)) {
			wake_unlock(&chg->asus_chrg_detect_wake_lock);
		}
	    return;
	...
	rc = vote(chg->chg_disable_votable, DCP_VOTER, true, 0);
	if (rc < 0) {
		smblib_err(chg, "Couldn't disable charging rc=%d\n", rc);
		if(wake_lock_active(&chg->asus_chrg_detect_wake_lock)) {
			wake_unlock(&chg->asus_chrg_detect_wake_lock);
	...
	msleep(10);
	if (chg->amux4_vadc_dev) {
		qpnp_vadc_read(chg->amux4_vadc_dev, VADC_AMUX_THM4, &ausu_vadc_result);
		vdm_1 = ausu_vadc_result.physical;
	}

	//GP1 High?
	gpio_direction_output(ASUS_ADAPTER_CONTROL, 1);
	/*
	smblib_err(chg," www gpio %d, val %d\n",
		ASUS_ADAPTER_CONTROL, gpio_get_value(ASUS_ADAPTER_CONTROL));
	*/
	msleep(10);
	if (chg->amux4_vadc_dev) {
		qpnp_vadc_read(chg->amux4_vadc_dev, VADC_AMUX_THM4, &ausu_vadc_result);
	...
	gpio_direction_output(ASUS_CHARGER_SEL, 0);
	/*
	smblib_err(chg," www id done, gpio %d, val %d\n",
		ASUS_ADAPTER_CONTROL, gpio_get_value(ASUS_ADAPTER_CONTROL));
	*/
	CHG_DBG("%s vdm_1=%lld vdm_2=%lld\n", __func__, vdm_1, vdm_2);
	vote(chg->fcc_votable, BATT_PROFILE_VOTER, false, 0);
	vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, false, 0);
	if (vdm_1 < 300000)
	{
//Bug318469,zhaosidong.wt,MODIFY,20171113,adjust adc data range	
		if (((vdm_2 > 290000) && (vdm_2 < 450000)) || ((vdm_2 > 580000 && vdm_2 < 800000)))
		{
			vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 2000000);
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 2000000);
			smblib_set_prop_fcc_delta(chg,500000); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
			asus_support_fast_charge = true; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
			CHG_DBG_EVT( "zy: charger is Asus 5V/2A  set  2A (support_fast_charge, %s-like)\n", 
				vdm_2 < 450000 ? "200K":"750K");
		} else {
			vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1000000);;
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1000000);
			smblib_set_prop_fcc_delta(chg,0); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
			asus_support_fast_charge = false; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
			CHG_DBG_EVT( "zy: charger is other 5V/1A  set  1A \n");
		}
		...
		smblib_set_prop_fcc_delta(chg,500000); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
		asus_support_fast_charge = true; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
		CHG_DBG_EVT( "zy: charger is Asus PB 5V/2A  set  2A (support_fast_charge)\n");
	} else {
		vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1000000);
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1000000);
		smblib_set_prop_fcc_delta(chg,0); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
	...
	if (gpio_is_valid(ASUS_CHARGER_SEL))
		gpio_free(ASUS_CHARGER_SEL);
	if (gpio_is_valid(ASUS_ADAPTER_CONTROL))
		gpio_free(ASUS_ADAPTER_CONTROL);

	rc = vote(chg->chg_disable_votable, DCP_VOTER, false, 0);
	if (rc < 0) {
		smblib_err(chg, "Couldn't enable charging rc=%d\n", rc);
	}
...
//+Bug334281,zhaosidong.wt,MODIFY,20180109,delay detection charger id
void asus_charger_id_detected_backup(struct smb_charger *chg)
{
	int64_t vdm_1, vdm_2 = 0;
	int rc = 0;
	int ret = 0;
    ...
    /*baidabin add for detect the country code*/
    if (false == asus_country_code_is_br) {
        ret = asus_country_code_read(buff);
	    pr_err("read buf buff:%s\n",buff);
		if (ret < 0) {
			pr_err("%s, read asus country code fail, buf:%s\n", __func__, buff);
		...
		if (!strncmp(buff, "BR", 2)) {
			asus_country_code_is_br = true;	
			pr_err("%s, The country code is BR, not support id detected.\n", __func__);
		} 		   
	}
	/*For BR, Set Current is 2A*/
	if (true == asus_country_code_is_br) {
		vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 2000000);
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 2000000);
		asus_support_fast_charge = true; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
		smblib_set_prop_fcc_delta(chg,500000); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
		smblib_err(chg, "The country is BR, Set current as 2A.\n");
		if(wake_lock_active(&chg->asus_chrg_detect_wake_lock)) {
			wake_unlock(&chg->asus_chrg_detect_wake_lock);
		}
	    return;
	...
	//
	gpio_request(ASUS_CHARGER_SEL, NULL);
	gpio_direction_output(ASUS_CHARGER_SEL, 1);
	rc = vote(chg->chg_disable_votable, DCP_VOTER, true, 0);
	if (rc < 0) {
		smblib_err(chg, "Couldn't disable charging rc=%d\n", rc);
		if(wake_lock_active(&chg->asus_chrg_detect_wake_lock)) {
	...
	msleep(100);
	//GP1 Low?
	gpio_request(ASUS_ADAPTER_CONTROL, NULL);
	gpio_direction_output(ASUS_ADAPTER_CONTROL, 0);
	msleep(100);
	if (chg->amux4_vadc_dev) {
		qpnp_vadc_read(chg->amux4_vadc_dev, VADC_AMUX_THM4, &ausu_vadc_result);
	...
	gpio_direction_output(ASUS_ADAPTER_CONTROL, 0);
	gpio_direction_output(ASUS_CHARGER_SEL, 0);
	smblib_err(chg, "zy: %s vdm_1=%lld vdm_2=%lld\n", __func__, vdm_1, vdm_2);
	vote(chg->fcc_votable, BATT_PROFILE_VOTER, false, 0);
	vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, false, 0);
	if (vdm_1 < 300000)
	{
//Bug318469,zhaosidong.wt,MODIFY,20171113,adjust adc data range	
		if (((vdm_2 > 290000) && (vdm_2 < 450000)) || ((vdm_2 > 580000 && vdm_2 < 800000)))
		{
			...
			smblib_set_prop_fcc_delta(chg,500000); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
			asus_support_fast_charge = true; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
			smblib_err(chg, "zy: charger is Asus 5V/2A  set  2A \n");
		} else {
			vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1000000);;
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1000000);
			smblib_set_prop_fcc_delta(chg,0); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
			asus_support_fast_charge = false; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
			smblib_err(chg, "zy: charger is other 5V/1A  set  1A \n");
		}
	} else if (vdm_1 > 1000000) {
		vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 2000000);
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 2000000);
		smblib_set_prop_fcc_delta(chg,500000); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
		asus_support_fast_charge = true; //Bug-321484,baidabin.wt,MODIFY,2017-10-26,Modify for charger thermal 
		smblib_err(chg, "zy: charger is Asus PB 5V/2A  set  2A \n");
	} else {
		vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1000000);
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1000000);
		smblib_set_prop_fcc_delta(chg,0); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, modify the current Compensation
...
int demo_app_current_feature(struct smb_charger *chg, u8 bat_capacity){
	...
	if (demo_app_property_flag) {
		...
		//if(demo_app_state_flag){
			...
			CHG_DBG_EVT("demo app status %d\n",demo_chg_status);
				
		//}
	}
	return  demo_chg_status;
	
}



static void country_code_detect_work(struct work_struct *work)
{
   	struct delayed_work *dwork = to_delayed_work(work);
	struct smb_charger *chg = container_of(dwork,struct smb_charger, country_code_detect_work);
	asus_dcp_detect_flag = 1;
	asus_charger_id_detected(chg);
	asus_dcp_detect_flag = 0;
}
...
{
	int result_State;
	int hyteresis[JEITA_STATE_LARGER_THAN_600+1] ={0,30,130,230,470,470,570};
	...
	switch(old_State){
		...
		case JEITA_STATE_LESS_THAN_0:
			if((result_State == old_State+1) && batt_tempr <= hyteresis[old_State])
...
#define CURRENT_LIMIT_TRIGGER_UV	4250000
#define CURRENT_LIMIT_RELEASE_UV	4200000
#define HIGH_TEMP_VOLT_THD		4100000
#define HIGH_TEMP_VOLT_BUFFER		30000
#define LOG_PRINT_PERIOD	1
#define FV_4P064V		0x4D
#define FV_4P1V			0x52
#define FV_4P357V		0x74
...
static void asus_temp_voltage_protect_work(struct work_struct *work)
{
    struct delayed_work *dwork = to_delayed_work(work);
    struct smb_charger *chg = container_of(dwork,struct smb_charger, asus_temp_voltage_protect_work);
    union power_supply_propval val_volt;
    union power_supply_propval val_temp;
    union power_supply_propval val_current; //+Bug 331298 ,baidabin.wt,MODIFIED,2017-12-21, for debug info
    union power_supply_propval val_usb_volt; //+ExtB 1130283, baidabin.wt,ADDED,2017-12-26 for debug info
    union power_supply_propval val_chg_status={0,};
    union power_supply_propval usb_online={0,};
    union power_supply_propval dc_online={0,};
    const struct apsd_result *apsd_result;
    int batt_temp = 0;
    int batt_voltage = 0;
    int batt_current = 0;//+Bug 331298 ,baidabin.wt,MODIFIED,2017-12-21, for debug info
    int usb_voltage = 0;//+ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for debug info
    int rc = 0;

	int bat_capacity = 40;
	static int cnt=LOG_PRINT_PERIOD -1;
	static int state = JEITA_STATE_INITIAL;
	u8	charging_enable= 0;
	u8	FV_CFG_reg_value= g_fv_setting; //init val 
	u8	FCC_reg_value= 0;
	u8	regval =0;
	
	//+Bug 339642 ,baidabin.wt,MODIFIED,2018-02-03, for crush issue
	if (!chg->bms_psy)
	{
		CHG_DBG_E("chg->bms_psy is null , return!\r\n");
		return;
	}
	//-Bug 339642 ,baidabin.wt,MODIFIED,2018-02-03, for crush issue

    rc = power_supply_get_property(chg->bms_psy,POWER_SUPPLY_PROP_VOLTAGE_NOW, &val_volt);
    batt_voltage = val_volt.intval;

    rc = power_supply_get_property(chg->bms_psy,POWER_SUPPLY_PROP_TEMP, &val_temp);
    batt_temp = val_temp.intval;

	//WeiYu: apply fake temp for local test jeita
	if(asus_fake_temp != FAKE_TEMP_DISABLE){
		batt_temp = asus_fake_temp;
		CHG_DBG("%s:report fake temp: %d\n", __func__, asus_fake_temp);
	}
	
    //+Bug 331298 ,baidabin.wt,MODIFIED,2017-12-21, for debug info
    rc = power_supply_get_property(chg->bms_psy,POWER_SUPPLY_PROP_CURRENT_NOW, &val_current);
    batt_current = val_current.intval;

    rc = power_supply_get_property(chg->bms_psy,POWER_SUPPLY_PROP_CAPACITY, &val_current);
	bat_capacity = val_current.intval;
	
//+ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for debug info
    rc = smblib_get_prop_usb_voltage_now(chg, &val_usb_volt);
    usb_voltage = val_usb_volt.intval;

	smblib_get_prop_batt_status(chg,&val_chg_status);

	smblib_read(chg, USBIN_CURRENT_LIMIT_CFG_REG,&regval);

    apsd_result = smblib_get_apsd_result(chg);


	state = smbchg_jeita_judge_state(state, batt_temp);


	if(++cnt == LOG_PRINT_PERIOD ){ //WeiYu: reduce log
		cnt =0;
	    	CHG_DBG("BAT_TEMP %d, VBAT %d mv, BAT_CURR %d ma, VBUS %d mv, State %s, APSD_BIT 0x%x, icl 0x%x, Jstate %d%s\n",
				batt_temp,batt_voltage/1000,batt_current/1000,usb_voltage/1000,
				batt_status_text[val_chg_status.intval] ,apsd_result->bit, regval, 
				state, state == JEITA_STATE_RANGE_200_to_500? "(good)":"");

	}//-Bug 331298 ,baidabin.wt,MODIFIED,2017-12-21, for debug info
//-ExtB 1130283 ,baidabin.wt,ADDED,2017-12-26 for debug info
smblib_get_prop_usb_online(chg,&usb_online);
smblib_get_prop_dc_online(chg,&dc_online);
if(usb_online.intval || dc_online.intval || chg->real_charger_type == POWER_SUPPLY_TYPE_USB){
//if(0){
    if (apsd_result->bit == DCP_CHARGER_BIT){
		...
		case JEITA_STATE_LESS_THAN_0:
			...
			FV_CFG_reg_value =  g_fv_setting; 
			FCC_reg_value = SMBCHG_FAST_CHG_CURRENT_VALUE_850MA;	
			break;
			
		case JEITA_STATE_RANGE_0_to_100:
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1400000);
				vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1400000);
		...
		case JEITA_STATE_RANGE_200_to_500:
			charging_enable = EN_BAT_CHG_EN_COMMAND_TRUE;
			if ((batt_voltage > CURRENT_LIMIT_TRIGGER_UV) && (false == asus_high_voltage_limit_current))
			{  
		    		CHG_DBG("battery voltage is more than %d mv.\n",CURRENT_LIMIT_TRIGGER_UV/1000);
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1500000);
				vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1500000);  
				asus_high_voltage_limit_current =true;
			}
			else if ((batt_voltage < CURRENT_LIMIT_RELEASE_UV) && (true == asus_high_voltage_limit_current))
			{
		    		CHG_DBG("battery voltage is less than %d mv.\n",CURRENT_LIMIT_RELEASE_UV);
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 2000000);
				vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 2000000);  
				asus_high_voltage_limit_current =false;
			}
			smblib_masked_write(chg, FLOAT_VOLTAGE_CFG_REG,
					FLOAT_VOLTAGE_SETTING_MASK, g_fv_setting);//4  	
			asus_high_temp_limit_voltage = false;		
			break;
		case JEITA_STATE_RANGE_500_to_600:

			if(batt_voltage >= HIGH_TEMP_VOLT_THD){
				asus_high_temp_limit_voltage = true;
				charging_enable = EN_BAT_CHG_EN_COMMAND_FALSE;
				smblib_masked_write(chg, FLOAT_VOLTAGE_CFG_REG,
						FLOAT_VOLTAGE_SETTING_MASK, FV_4P357V);//4  
						
			}else{
				//bat_capacity < x for safty
				if(!asus_high_temp_limit_voltage || bat_capacity < 40){
					charging_enable = EN_BAT_CHG_EN_COMMAND_TRUE;
					smblib_masked_write(chg, FLOAT_VOLTAGE_CFG_REG,
							FLOAT_VOLTAGE_SETTING_MASK, FV_4P064V);//4  
					vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1500000);
					vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1500000);  						
				}
			}
			CHG_DBG("%s: 50 <= temperature < 60, chg_en 0x%x , high_temp_limit_voltage %d\n",
				__func__,charging_enable, asus_high_temp_limit_voltage);
		...
		smblib_masked_write(chg, FLOAT_VOLTAGE_CFG_REG,
				FLOAT_VOLTAGE_SETTING_MASK, FV_CFG_reg_value);
		
	}else{ //non-dcp

		switch (state) {
		case JEITA_STATE_LESS_THAN_0:
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 900000);
	...
	CHG_DBG("usb %d, dc %d, flag 0x%x, FV 0x%x\n",
		usb_online.intval,dc_online.intval,charging_enable,FV_CFG_reg_value);
}

	//smblib_masked_write(smbchg_dev, FAST_CHARGE_CURRENT_CFG_REG,
...
/*
	...
	{
        //+Bug 331298 ,baidabin.wt,MODIFIED,2017-12-21, for debug info
		//CHG_DBG("%s start work,asus_support_fast_charge is %d\n",__func__, (int)asus_support_fast_charge);
		//+Bug 331298 ,baidabin.wt,MODIFIED,2017-12-21, for debug info


		//+Bug 331298 ,baidabin.wt,MODIFIED,2017-12-21, for low temp current
		if (true == asus_support_fast_charge)
		{
			//+Bug 338262,baidabin.wt,MODIFY,2018-02-02,Modify adjust soft cold threhold
			if (batt_temp < 100)
			{
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1400000);
				vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1400000);
        	...
        	//
		if ( ((batt_temp >= 200)&&(batt_temp < 500))){
			if ((batt_voltage > CURRENT_LIMIT_TRIGGER_UV) && (false == asus_high_voltage_limit_current))
			{  
		    		CHG_DBG("battery voltage is more than %d mv.\n",CURRENT_LIMIT_TRIGGER_UV/1000);
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1500000);
...
*/
	...
	demo_app_current_feature(chg, bat_capacity);


	
    schedule_delayed_work(&chg->asus_temp_voltage_protect_work, msecs_to_jiffies(ASUS_BATTERY_TEMP_VOLTAGE_MS)); 
}
//-Bug 329838,baidabin.wt,MODIFY,2017-12-15, Add for voltage and temp protect
//-Bug321118 baidabin.wt,MODIFY,20171129,modify country code detect issue
...
//+Bug305852, zk-zhouying.wt, ADD, 2017-10-27, add asus adapter check
	if (apsd_result->bit == DCP_CHARGER_BIT) {
		if(!wake_lock_active(&chg->asus_chrg_detect_wake_lock)) {
            wake_lock(&chg->asus_chrg_detect_wake_lock);
        }
	...
	gpio_direction_output(ASUS_ADAPTER_CONTROL, 0);
	/*
	smblib_err(chg," www work gpio %d, val %d\n",
		ASUS_ADAPTER_CONTROL, gpio_get_value(ASUS_ADAPTER_CONTROL));
	*/
	//EN High
	gpio_request(ASUS_CHARGER_SEL, NULL);
	...
	/*
	smblib_err(chg," www work gpio %d, val %d\n",
		ASUS_CHARGER_SEL, gpio_get_value(ASUS_CHARGER_SEL));
	*/
	//WeiYu switch DPDM ---
	
        schedule_delayed_work(&chg->country_code_detect_work, msecs_to_jiffies(ASUS_COUNTRY_CODE_DETECT_MS));
	}
//-Bug305852, zk-zhouying.wt, ADD, 2017-10-27, add asus adapter check
//-Bug334281,zhaosidong.wt,MODIFY,20180109,delay detection charger id
//+Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
    if ((FLOAT_CHARGER_BIT == apsd_result->bit) && (false == asus_float_charger_rerun))
    {
        pr_err("float charger is detect , rerun the test.\r\n");
        //smblib_rerun_apsd(chg);


		
        schedule_delayed_work(&chg->asus_float_charger_rerun_work, msecs_to_jiffies(ASUS_FLOAT_CHARGER_RERUN_MS)); 
    }
//-Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: apsd-done rising; %s detected\n",
		   apsd_result->name);
	CHG_DBG_EVT("IRQ: apsd-done rising; %s detected,\n",
		   apsd_result->name);
}
//+Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
static void asus_float_charger_rerun_work(struct work_struct *work)
{
    struct delayed_work *dwork = to_delayed_work(work);
    struct smb_charger *chg = container_of(dwork,struct smb_charger, asus_float_charger_rerun_work);

    pr_err("asus_float_charger_rerun_work is enter.\r\n");
    asus_float_charger_rerun = true;
    smblib_rerun_apsd(chg);
}
//-Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
irqreturn_t smblib_handle_usb_source_change(int irq, void *data)
{
	...
	int rc = 0;
	u8 stat;
	const struct apsd_result *apsd;	//Bug314759,zhaosidong.wt,MODIFY,20171031,synchronize the modification of the charging problem
...
//+Bug314759,zhaosidong.wt,MODIFY,20171031,synchronize the modification of the charging problem	
		...
		if ((apsd->pst == POWER_SUPPLY_TYPE_USB_DCP) ||
			...
			(apsd->pst == POWER_SUPPLY_TYPE_USB_CDP)) {            //Bug333324,zhaosidong.wt,MODIFY,20180102,do not rerun apsd with cdp charging type
			smblib_dbg(chg, PR_MISC, "apsd_result = 0x%02x\n, DCP don't need rerun apsd.", apsd->pst);
		} else {		
			smblib_rerun_apsd(chg);
			return IRQ_HANDLED;
		}
...
#ifdef WT_COMPILE_FACTORY_VERSION
int into_fastmmi_mode(struct smb_charger *chg)
{
    int ret;
    ...
    if(cmdline_fastmmi != NULL) {
		...
		if(ret == 0) {
			pr_err("into charger modes\n");
            return 2;/* charger mode*/
		}
    }
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, abnormal_detect.work);
	int rc, soc, ret;
	union power_supply_propval val = {0,};
	...
	if(!(ret == 1)) {
		if(ret == 2) {
			if(soc >= 0 && soc < 60) {
			    ...
			    rc = vote(chg->chg_disable_votable, DCP_VOTER, false, 0);
				if (rc < 0) {
				    dev_err(chg->dev,"vote method Couldn't enable charging: rc = %d\n", rc);
			    } 
			} else if(soc >= 70 && soc <= 100) {
		        val.intval = 1;
			    rc = vote(chg->chg_disable_votable, DCP_VOTER, true, 0);
			    if (rc < 0) {
				    dev_err(chg->dev,"vote method Couldn't disable charging: rc = %d\n", rc);
			    }
		    }		
		} else {
		    if(soc >= 0 && soc < 60) {
	...
	schedule_delayed_work(&chg->abnormal_detect,msecs_to_jiffies(20000));
}
#endif
//-Bug317384,zhaosidong.wt,ADD,20171108,add battery level control for ATO version
//+Bug336640,zhaosidong.wt,ADD,20180124,add for usb thermal alert
int into_charger_mode(struct smb_charger *chg)
{
    int ret;
    char *cmdline_fastmmi=NULL;
    char *temp;
...
//+Bug317384,zhaosidong.wt,ADD,20171108,add battery level control for ATO version
#ifdef WT_COMPILE_FACTORY_VERSION	
	INIT_DELAYED_WORK(&chg->abnormal_detect, charger_abnormal_detect_work);
#endif
//-Bug317384,zhaosidong.wt,ADD,20171108,add battery level control for ATO version
    INIT_DELAYED_WORK(&chg->country_code_detect_work, country_code_detect_work); //Bug321118 baidabin.wt,MODIFY,20171129,modify country code detect issue
    INIT_DELAYED_WORK(&chg->asus_temp_voltage_protect_work, asus_temp_voltage_protect_work);//Bug 329838,baidabin.wt,MODIFY,2017-12-15, Add for voltage and temp protect
    INIT_DELAYED_WORK(&chg->asus_float_charger_rerun_work, asus_float_charger_rerun_work);//Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
//+Bug336640,zhaosidong.wt,MODIFY,20180124,add for usb thermal alert
    INIT_DELAYED_WORK(&chg->usb_temp_det_work, usb_temp_detect_callback);
    chg->pre_otgchg_temp_enable = 1;
	chg->pre_usbchg_temp_enable = 1;
	chg->thermal_offline=0;
	chg->poweroff_charger_mode=into_charger_mode(chg);
//-Bug336640,zhaosidong.wt,MODIFY,20180124,add for usb thermal alert
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		cancel_work_sync(&chg->rdstd_cc2_detach_work);
		cancel_delayed_work_sync(&chg->hvdcp_detect_work);
		cancel_delayed_work_sync(&chg->country_code_detect_work); //+Bug321118 baidabin.wt,MODIFY,20171129,modify country code detect issue
		cancel_delayed_work_sync(&chg->asus_temp_voltage_protect_work); //+Bug 329838,baidabin.wt,MODIFY,2017-12-15, Add for voltage and temp protects
		cancel_delayed_work_sync(&chg->asus_float_charger_rerun_work); //+Bug 330703,baidabin.wt,MODIFY,2017-12-20, Add for float charger rerun
