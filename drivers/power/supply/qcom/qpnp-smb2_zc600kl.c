		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (tmp == 0) {
		...
		CHG_DBG("%s: CHARGING_ENABLE_CMD_REG = 0\n", __func__);
	} else if (tmp == 1) {
		smblib_masked_write(smbchg_dev, CHARGING_ENABLE_CMD_REG,
			CHARGING_ENABLE_CMD_BIT, 1);
		demo_app_property_flag = true;
		CHG_DBG("%s: CHARGING_ENABLE_CMD_REG = 1\n", __func__);
    }
...
static ssize_t charger_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	u8 val=0;
	smblib_read(smbchg_dev,CHARGING_ENABLE_CMD_REG, &val);
	
       return sprintf(buf, "0x%x\n", val);
}


static DEVICE_ATTR(charger_info, 0664, charger_info_show, charger_info_store);

static ssize_t test_therm_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	int tmp = 250;
...
static ssize_t test_therm_show(struct device *dev, struct device_attribute *attr, char *buf)
{
