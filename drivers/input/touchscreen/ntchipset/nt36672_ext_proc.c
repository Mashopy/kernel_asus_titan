...
#define NVT_FWVER_INFO "fwver_info"

static struct proc_dir_entry *NVT_Custom_Gesture_entry;
static struct proc_dir_entry *NVT_Custom_Glove_entry;
static struct proc_dir_entry *NVT_proc_fwver_info_entry;

long i64_GESTURE = 0;
int nvt_gesture_enable=0;
char msg[20];
static int nvtc_show_proc(struct seq_file *m,void *v)
{
...
bool nvt_gesture_check(void)
{
	return (((i64_GESTURE & 0x100) != 0) || (i64_GESTURE & 0x002) || (i64_GESTURE & 0x001));
}
EXPORT_SYMBOL(nvt_gesture_check);

static int nvtc_open_proc(struct inode *inode, struct  file *file)
{
    return single_open(file, nvtc_show_proc, NULL);
}
...
static int glove_show_proc(struct seq_file *m,void *v)
{
...
static int glove_open_proc(struct inode *inode, struct  file *file)
{
    return single_open(file, glove_show_proc, NULL);
}

static const struct file_operations nvtc_glove_fops = {
	.owner = THIS_MODULE,
	.open = glove_open_proc,
	.release = single_release,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int nvt_fwver_info_show_proc(struct seq_file *m,void *v)
{
	printk("ntchipset: ID:%x,fw:%x\n ", ts->nvt_pid, ts->fw_ver);
	seq_printf(m, "ntchipset: ID:%x,fw:%x\n", ts->nvt_pid, ts->fw_ver);
	return 0;
}

static int nvt_fwver_info_open_proc(struct inode *inode, struct  file *file)
{
    return single_open(file, nvt_fwver_info_show_proc, NULL);
}

static const struct file_operations nvtc_fwver_info_fops = {
	.owner = THIS_MODULE,
	.open = nvt_fwver_info_open_proc,
	.release = single_release,
	.read = seq_read,
	.llseek = seq_lseek,
};
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_fwver_info_entry = proc_create(NVT_FWVER_INFO, 0444, NULL,&nvtc_fwver_info_fops);
	if (NVT_proc_fwver_info_entry == NULL) {
		NVT_ERR("create proc/fwver_info Failed!\n");
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/fwver_info Succeeded!\n");
	}
