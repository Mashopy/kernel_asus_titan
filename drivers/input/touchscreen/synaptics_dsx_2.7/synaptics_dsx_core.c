...
extern char synap_firmware_ver[64];
#ifdef CONFIG_FB
...
static int synap_show_proc(struct seq_file *m,void *v)
{
	printk("gestures_flag= 0x%03X\n",(int)gestures_flag);
	seq_printf(m, "GESTURE=0x%03x\n", (unsigned int)gestures_flag);
	return 0;
}
...
bool synap_gesture_check(void)
{
	return (((gestures_flag & 0x100) != 0)  || (gestures_flag & _GESTURE_DOUBLE_CLICK) || (gestures_flag & _GESTURE_SLIDE));
}
EXPORT_SYMBOL(synap_gesture_check);

static int synap_open_proc(struct inode *inode, struct  file *file)
{
    return single_open(file, synap_show_proc, NULL);
}

static const struct file_operations synap_gesture_fops = {
	.owner = THIS_MODULE,
	.open = synap_open_proc,
...
static int synap_glove_show_proc(struct seq_file *m,void *v)
{
	printk("glove_enable =%d\n ",glove_enable);
	seq_printf(m, "glove flag=%d\n", glove_enable);
	return 0;
}
...
static int synap_glove_open_proc(struct inode *inode, struct  file *file)
{
    return single_open(file, synap_glove_show_proc, NULL);
}

static const struct file_operations synap_glove_fops = {
	.owner = THIS_MODULE,
	.open = synap_glove_open_proc,
	.release = single_release,
	.read = seq_read,
	.llseek = seq_lseek,
...
static struct proc_dir_entry *synap_fwver_info_entry;
#define SYNAP_FWVER_INFO "fwver_info"

static int synap_fwver_info_show_proc(struct seq_file *m,void *v)
{
	printk("synap_firmware_ver : %s\n",synap_firmware_ver);
	seq_printf(m, "%s\n" ,synap_firmware_ver);
	return 0;
}

static int synap_fwver_info_open_proc(struct inode *inode, struct  file *file)
{
    return single_open(file, synap_fwver_info_show_proc, NULL);
}

static const struct file_operations synap_fwver_info_fops = {
	.owner = THIS_MODULE,
	.open = synap_fwver_info_open_proc,
	.release = single_release,
	.read = seq_read,
	.llseek = seq_lseek,
};
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (g_asus_lcdID == 2){
		pr_err("[synaptic][touch] lcdid= %d  is novatek touch \n",g_asus_lcdID);
		return 0;
	}
...
#endif
synap_fwver_info_entry = proc_create(SYNAP_FWVER_INFO, 0444, NULL,&synap_fwver_info_fops);
	if (synap_fwver_info_entry == NULL) {
			dev_err(&pdev->dev,"create proc/%s Failed!\n",SYNAP_FWVER_INFO);
