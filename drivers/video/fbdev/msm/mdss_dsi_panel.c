...
#define ASUS_ZE554KL_TOUCH_READY       1
#define ASUS_ZE620KL_TOUCH_READY       1

// All project common interfaces
...
void asus_lcd_cabc_value_restore(int unlocked);

static struct workqueue_struct *asus_lcd_delay_workqueue;

// OEM project only interfaces
...
// for Touch suspend/resume when touch porting finished
#if ASUS_ZE554KL_TOUCH_READY
...
#else
// ZC600KL work func, for cabc initial
static void asus_lcd_delay_work_func(struct work_struct *);
static DECLARE_DELAYED_WORK(asus_lcd_delay_work, asus_lcd_delay_work_func);
#endif /* end of ifndef ASUS_ZC600KL_PROJECT */
//ASUS_BSP: ---
...
/* backlight DCS command 1 parameter */
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (!pinfo->cont_splash_enabled) {
			...
			printk("[Display] panel reset enabling.\n");
...
#else
	// ZC600KL setting CABC value for the first boot
...
#if defined(ASUS_ZE554KL_PROJECT) && ASUS_ZE554KL_TOUCH_READY
	if (g_asus_lcdID != TITAN_LCD_TM) {
		asus_rmi4_resume(); //for ZE554KL synaptic touch resume
...
#if defined(ASUS_ZE620KL_PROJECT) && ASUS_ZE620KL_TOUCH_READY
	fts_ts_resume(); //for ZE620KL focal touch resume
#endif

#if defined(ASUS_ZE554KL_PROJECT) && ASUS_ZE554KL_TOUCH_READY
	if (g_asus_lcdID == TITAN_LCD_TM) {
		fts_ts_resume(); //for ZE554KL focal touch resume
	}
#endif

#else //ZC600KL
	/* Restore cabc value unlocked due to we're in on cmd sequence */
	asus_lcd_cabc_value_restore(1);
#endif
...
void asus_lcd_cabc_value_restore(int unlocked)
{
	char cabc_prefix = 0x00;

#ifdef ASUS_ZC600KL_PROJECT
	if (g_asus_lcdID == ZC600KL_LCD_TD4310) {
		cabc_prefix = 0x80;
	}
#endif

	asus_lcd_cabc_mode[1] |= cabc_prefix;

	printk("[Display] Setting CABC reg to 0x%x, unlocked: %d\n", asus_lcd_cabc_mode[1], unlocked);
	if (unlocked)
		asus_tcon_cmd_set_unlocked(asus_lcd_cabc_mode, ARRAY_SIZE(asus_lcd_cabc_mode));
	else
...
static ssize_t asus_lcd_cabc_proc_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
	...
	if(strncmp(messages, "0", 1) == 0)  //off
		asus_lcd_cabc_mode[1] = 0x00;
	else if(strncmp(messages, "1", 1) == 0) //ui
		asus_lcd_cabc_mode[1] = 0x01;
	else if(strncmp(messages, "2", 1) == 0) //still
		asus_lcd_cabc_mode[1] = 0x02;
	else if(strncmp(messages, "3", 1) == 0) //moving
		asus_lcd_cabc_mode[1] = 0x03;

	asus_lcd_cabc_value_restore(0);
                    ...
                    size_t count, loff_t *ppos)
{
	...
	ssize_t ret = 0;
	char *buff;
	const char cabc_mask = 0x0f; //take 0-3 bits
	int cabc_current_value = asus_lcd_cabc_mode[1] & cabc_mask;
	...
	int ndx)
{
	...
	// All project proc interfaces
	proc_create(ASUS_LCD_PROC_CABC,        0777, NULL, &asus_lcd_cabc_proc_ops);
	proc_create(ASUS_LCD_PROC_REGISTER_RW, 0666, NULL, &asus_lcd_reg_rw_ops);
